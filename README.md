## Artifacts for "All in One: Design, Verification, and Implementation of Latency-Optimal Read Atomic Transactions"

- The "lora" folder contains:
  - the Maude specifications of LORA, RAMP-F, and ALT;
  - the transformed probabilistic models of LORA, RAMP-F, and ALT;
  - the generated distriubted Maude implementations of LORA and RAMP-F;
  - the implementation of the workload generator replicated in each corresponding sub-folder;
  - the formal specifications of the consistency and performance properties replicated in each corresponding sub-folder.
 
- The "ext-cat" folder contains the extended CAT model checker.

- The "monitor" folder contains the implementation of the monitoring mechanism.

- The "deploy" folder contains the deploying tool.


